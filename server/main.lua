...
AddEventHandler('esx_blackmarket:buyItem', function(itemName, amount, zone)
	...
	if xPlayer.getMoney() >= preco then
		...
		if sourceItem.weight ~= -1 and (sourceItem.count + amount) > sourceItem.weight then
			...
			TriggerClientEvent('esx:showNotification', _source, _U('bought', amount, itemLabel, preco))
		end
	else
		local missingMoney = preco - xPlayer.getMoney()
